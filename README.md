...
|------|--------|---------------|-------------|
| **Via Stitching** | ✅ Active | [VIA_STITCHING.md](docs/VIA_STITCHING.md) | Ensures critical signal vias have nearby GND return vias within configurable distance (default 2mm). Prevents EMI radiation and maintains signal integrity. |
| **Decoupling Capacitors** | ✅ Active | [DECOUPLING.md](docs/DECOUPLING.md) | Verifies IC power pins have decoupling capacitors within configurable distance (default 3mm). Uses **smart net matching** - only checks capacitors on the same power rail. Includes visual arrows to nearest cap. |
| **Ground Plane Continuity** | ✅ Active | [GROUND_PLANE.md](docs/GROUND_PLANE.md) | Verifies continuous ground plane underneath and around high-speed traces. Features: **performance optimized** (5-10× faster), **progress dialog**, **polygon area filtering**, **preferred layer priority**, **via/pad clearance ignore**. Checks for gaps under trace (default 0.5mm sampling) and clearance zone around trace (default 1mm). |
| **Signal Integrity** | ✅ Phase 1/2 | [IMPEDANCE_ALGORITHM.md](docs/IMPEDANCE_ALGORITHM.md)<br>[.github/instructions/signal-integrity.instructions.md](.github/instructions/signal-integrity.instructions.md) | **Phase 1/2 implemented** (~1,400 LOC): (1) Net length limits, (2) Exposed trace detection, (3) Unconnected via pads, (4) Trace-to-plane edge distance, **(5) Trace-to-board edge distance (EMI + manufacturing)**, (6) Unreferenced traces (multi-point sampling), (7) Critical net isolation (3W rule), (8) **Controlled impedance** (microstrip/stripline/differential, IPC-2141A formulas). **Phase 3/4 pending**: Stub length, crosstalk, differential pair matching, reference plane transitions. |
