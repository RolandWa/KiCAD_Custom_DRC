...
- ✅ **Centralized utilities**: `create_logger()`, `create_violation_group()` eliminate 170+ lines of duplicate code
- ✅ **Dependency injection**: Modules receive logger, group creator, visualization functions
- ✅ **Modular design**: Each checker is a separate class (150-700 lines)
- ✅ **Standardized markers**: All violations use EMC_* named groups on User.Comments layer
...
def check_decoupling(self, board, marker_layer, config):
    ...
    try:
        ...
        print(f"❌ ERROR in decoupling check: {e}")
        import traceback
        traceback.print_exc()
...
def check(self, ...):
    if not items_to_check:
        self.log("⚠️  No items found, skipping", force=True)
        return 0
...
if nearest_cap is None:
    self.log("❌ No capacitor found", force=True)
    return float('inf')

# ❌ BAD: Implicit truthiness (can fail for 0, [], False)
...
class YourChecker:
    ...
    def check(self, draw_marker_func: callable, draw_arrow_func: callable, 
        ...
        try:
            ...
            if not items:
                self.log("⚠️  No items found", force=True)
                return 0
...
try:
    ...
    print(f"⚠️  Could not import your_module: {e}")
    YourChecker = None
...
def check_your_feature(self, board, marker_layer, config):
    ...
    try:
        ...
        print(f"❌ ERROR in your_feature check: {e}")
        return 0
...
### ❌ DON'T: Implement log() in modules
```python
# ❌ OLD PATTERN (pre-v1.3.0)
class YourChecker:
...
5. **🔒 SECURITY CHECK (MANDATORY)**:
   ...
   git diff --cached | Select-String -Pattern "C:\\Users\\[^<]|OneDrive - |YourRealName|YourCompany"
   
   # Verify protected files are gitignored
   git status --ignored | Select-String -Pattern "sync_to_kicad.ps1|test_config.py"
   ```
   - ✅ **If no matches**: Safe to commit
   - ❌ **If matches found**: Remove sensitive data, use placeholders instead
...
```
KiCAD_Custom_DRC/
├── emc_auditor_plugin.py       # Main plugin (899 lines)
├── via_stitching.py            # Via stitching checker (221 lines)
├── decoupling.py               # Decoupling checker (188 lines)
├── emi_filtering.py            # EMI filtering checker (697 lines)
├── ground_plane.py             # Ground plane checker (495 lines)
├── clearance_creepage.py       # Clearance checker (639 lines)
├── emc_rules.toml              # Configuration
├── emc_icon.png                # Plugin icon
├── sync_to_kicad.ps1           # Deployment script
├── README.md                   # User documentation
├── CODE_REUSABILITY_ANALYSIS.md  # Code quality analysis
└── .copilot-instructions.md    # This file
```

---

## 🎨 Violation Marker Pattern
...
if self.config.get('draw_arrow_to_target', True):
    self.draw_arrow(self.board, start, end, "→ C15", self.marker_layer, violation_group)
...
- ❌ Real names, usernames, emails
- ❌ Company names, internal project names
- ❌ Absolute paths with usernames (`C:\Users\JohnDoe\...`)
- ❌ IP addresses, VPN endpoints
- ❌ API keys, passwords, tokens
- ❌ Real board names, customer data
- ❌ Test results from actual products

**ALWAYS Use:**
- ✅ Environment variables: `$env:USERPROFILE`, `%APPDATA%`
- ✅ Placeholders: `<repository_path>`, `<YourUsername>`, `<user_home>`
- ✅ Generic examples: `user@example.com`, `ExampleCorp`

### Pre-Commit Security Check (MANDATORY)

**Before every `git commit`, run these checks:**

```powershell
# 1. Check for sensitive patterns in tracked files
git grep -i "YourRealName\|YourCompany\|your.email@" HEAD

# 2. Search for usernames and absolute paths
git diff --cached | Select-String -Pattern "C:\\Users\\[^<]|OneDrive - "

# 3. Verify .gitignore is protecting sensitive files
git status --ignored

# 4. Check staged changes don't include local configs
git diff --cached --name-only | Select-String -Pattern "sync_to_kicad.ps1|local|*.local"
```

**Expected Results (Safe to Commit):**
- ✅ No matches for personal names/emails/company
- ✅ Only generic placeholders like `<YourUsername>` in README
- ✅ Local files (`sync_to_kicad.ps1`, `test_config.py`) appear in "Ignored files" section
- ✅ Only production code files in staged changes

**Files Protected by .gitignore:**
```gitignore
# Personal configurations (NEVER commit)
sync_to_kicad.ps1        # Contains your local KiCad path
*.local.ps1              # Any local scripts
local_config.toml        # Local configuration overrides
test_config.py           # Test scripts with paths
...
**Use Copilot to check before commit:**
```
User: "check for sensitive information before commit and push to git"
...
2. Remove sensitive information or use placeholders
3. If already committed: `git reset HEAD~1` (undo last commit)
4. If already pushed: Contact repository owner, may need force push or history rewrite

### Template Files for Personal Paths

**Always provide `.template` versions:**
```powershell
# sync_to_kicad.ps1.template (committed to repo)
$PluginsDir = "CHANGE_THIS_TO_YOUR_KICAD_PLUGINS_PATH"

# User creates their own sync_to_kicad.ps1 (gitignored)
$PluginsDir = "C:\Users\ActualUser\Documents\KiCad\9.0\3rdparty\plugins"
```
...
10. **Security** - **ALWAYS run pre-commit security check** (see 🔒 Security & Privacy section)
...
- [ ] No sensitive data (run `git grep -i` checks)
- [ ] .gitignore protecting local files
- [ ] Only production code in staged changes
