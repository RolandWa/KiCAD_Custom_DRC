...
**Current Modules:**
- `clearance_creepage.py` - Electrical clearance/creepage verification (IEC60664-1/IPC2221)
- `via_stitching.py` - Via stitching verification for EMI reduction
- `decoupling.py` - Decoupling capacitor proximity verification for signal integrity
- `emi_filtering.py` - EMI filtering verification on interface connectors (USB, Ethernet, CAN, etc.)
...
✅ **No Code Duplication** - Utility functions defined once in main plugin  
✅ **Clean Separation** - Complex checks isolated in modules  
✅ **Shared Logging** - All modules append to same `report_lines` list  
✅ **Consistent Visualization** - All use same `draw_error_marker()` style  
✅ **Testability** - Modules can be unit tested with mocked functions  
✅ **Maintainability** - Update marker style in one place, affects all checks  
...
**Board Utilities:**
```python
def get_nets_by_class(self, board, class_name)  # Net Class → net names
```
...
self.log("⚠️  No nets assigned to voltage domains", force=True)
self.log("❌ VIOLATION: Clearance too small", force=True)
```
...
self.log(f"    ✓ Assigned {count} nets via Net Class")

# Warnings (always visible)
self.log("⚠️  No clearance table found, using defaults", force=True)

# Violations (always visible)
self.log(f"  ❌ VIOLATION: {domain_a} ↔ {domain_b}", force=True)
...
def check_your_feature(self, board, marker_layer, config):
    ...
    if YourChecker is None:
        self.log("⚠️  YourChecker module not available", force=True)
...
## 🔌 Using Main Plugin Utility Functions in Modules
...
def _parse_voltage_domains(self):
    for domain_config in voltage_domains:
        ...
        if nets_in_class:
            ...
            self.log(f"  ✓ Assigned {len(nets_in_class)} net(s) via Net Class")
        else:
            # Fallback to pattern matching
            self.log(f"  ⚠ No nets in Net Class '{domain_name}', trying patterns")
```
...
class YourChecker:
    ...
    def check(self, draw_marker_func, draw_arrow_func, get_distance_func):
        ...
        self.draw_arrow(self.board, start, end, "→ Target", layer, group)
```
