...
**AI Assistant Rules**:
- ❌ **DO NOT** suggest adding third-party libraries that would force a license change to GPL
- ❌ **DO NOT** recommend copyleft dependencies (GPL, LGPL, AGPL)
- ✅ **DO** use permissive licenses only (MIT, BSD, Apache 2.0)
- ✅ **DO** use Python standard library whenever possible
- ✅ **DO** implement custom code rather than GPL dependencies
...
- ❌ **Personal Information**: Real names, usernames, email addresses, phone numbers
- ❌ **Corporate Identifiers**: Company names, internal project names, organization details
- ❌ **File Paths**: Absolute paths containing usernames or company names (e.g., `C:\Users\JohnDoe\...`)
- ❌ **Network Information**: Internal URLs, IP addresses, server names, VPN endpoints
- ❌ **Credentials**: Passwords, API keys, tokens, certificates, SSH keys
- ❌ **Internal Documentation**: Screenshots with sensitive UI, proprietary diagrams, confidential reports

**Required Practices**:
- ✅ **Use Placeholders**: Replace specific paths with `<repository_path>`, `<kicad_plugins_path>`, `<user_home>`
- ✅ **Use Environment Variables**: `$env:USERPROFILE`, `$HOME`, `%APPDATA%` instead of hardcoded paths
- ✅ **Generic Examples**: Use example.com, user@example.com, generic company names in documentation
- ✅ **Template Files**: Provide `.template` files for user-specific configurations (e.g., `sync_to_kicad.ps1.template`)
- ✅ **Gitignore Local Configs**: Add user-specific files to `.gitignore` (e.g., `sync_to_kicad.ps1`, `*.local.ps1`)

**Commit Message Rules**:
- ❌ **Never mention** what sensitive data was removed (e.g., "Removed company name XYZ")
- ✅ **Use generic descriptions**: "Update paths to use environment variables", "Use generic placeholders"
...
**CRITICAL**: Follow this methodology for all code changes to maintain clean history and avoid conflicts.
...
**2. Make Changes**:
- Edit plugin files (`emc_auditor_plugin.py`, `emc_rules.toml`, etc.)
- Test changes in KiCad before committing
- Run sync script to copy to KiCad plugins directory:
  ```powershell
  .\sync_to_kicad.ps1
  ```
...
# Review actual changes (line-by-line diff)
git diff

# Check for sensitive data
git diff | Select-String -Pattern "(RWache|Rockwell|@.*\.com|Users\\)"
```

**4. Stage and Commit**:
```powershell
# Stage specific files (recommended)
git add emc_auditor_plugin.py emc_rules.toml README.md
...
**5. Push to Remote**:
```powershell
# Normal push (when no history rewrite)
git push origin main

# Force push (ONLY after history rewrite or cleanup)
git push origin main --force

# Safer alternative (fails if remote changed)
...
- Why this change was necessary (if not obvious)
```

**Good Examples**:
```
Add RC filter topology detection to EMI filtering

- Implemented RC T-filter (R-C-R) and RC Pi-filter (C-R-C) detection
- Extended filter hierarchy to support lossy EMI filtering
- Updated TOML configuration with RC filter examples
```

```
Fix via stitching distance calculation bug

- Changed from center-to-center to pad-edge distance
- Prevents false positives on large pads
...
❌ "WIP"                      # Not descriptive
❌ "Removed company data"     # Reveals sensitive info removal
```

### Synchronization Best Practices

**Daily Workflow**:
1. **Morning**: `git pull` to get latest changes
2. **Work**: Make changes, test locally
3. **Evening**: `git add` + `git commit` + `git push`

**Sync Script Usage**:
- Run `.\sync_to_kicad.ps1` **AFTER** editing files but **BEFORE** testing in KiCad
- This copies updated files from repository to KiCad plugins directory
- Always restart KiCad after running sync to reload plugin

**Handling Conflicts** (if `git pull` shows conflicts):
```powershell
...
# Option 3: Manually resolve (open file, edit conflict markers)
# Then:
git add <file>
...
**Currently**: Single `main` branch (simpler for solo development)

**When to Create Branches**:
- Feature development: `git checkout -b feature/new-check`
- Bug fixes: `git checkout -b fix/via-stitching-bug`
- Experiments: `git checkout -b experiment/alternative-algorithm`
...
### History Cleanup (Advanced)

**When to Clean History**:
- Accidentally committed sensitive data
- Repository became too large
- Want fresh start with single commit

**Full History Reset** (DESTRUCTIVE):
```powershell
...
git add -A
git commit -m "Fresh start: <description>"

# 3. Delete old main branch
git branch -D main

# 4. Rename temp branch to main
git branch -m main

# 5. Force push (overwrites remote history)
git push origin main --force

# 6. Clean up local refs
git reflog expire --expire=now --all
git gc --prune=now --aggressive
```

**⚠️ WARNING**: Force push rewrites history. Coordinate with collaborators before using.

### Repository Maintenance

**Regular Checks**:
- ✅ Run `git status` frequently to track uncommitted changes
- ✅ Review `.gitignore` to ensure local configs aren't tracked
- ✅ Keep commits small and focused (easier to review/revert)
- ✅ Test plugin in KiCad before pushing
...
**🚧 Configuration Ready (Implementation Pending):**
...
if config.get('draw_arrow_to_target', True):
    self.draw_arrow(
        ...
        "→ TARGET_REF",      # Arrow label (e.g., "→ C15")
        marker_layer,
        violation_group
    )
```
...
- Assigned nets using wildcard pattern `"*In*"` in Board Setup → Net Classes
...
```python
# ✅ CORRECT: Use centralized helper (handles substring matching)
net_list = self.get_nets_by_class(board, 'HIGH_VOLTAGE_DC')
for net_name in net_list:
    print(f"Found net: {net_name}")

# ❌ WRONG: Exact match fails with comma-separated classes
for net in board.GetNetInfo().NetsByName().values():
    if net.GetNetClassName() == 'HIGH_VOLTAGE_DC':  # Misses "HIGH_VOLTAGE_DC,Default"
        ...

# ❌ WRONG: Using non-existent Find() method
...
- [ ] Right-click marker → "Select Items in Group" selects all markers
...
❌ **WRONG:** Creating one group for all violations
```python
# Don't do this:
violation_group = pcbnew.PCB_GROUP(board)
board.Add(violation_group)
...
✅ **CORRECT:** Create individual group per violation
```python
# Do this:
for item in items:
...
❌ **WRONG:** Hardcoding marker dimensions
```python
